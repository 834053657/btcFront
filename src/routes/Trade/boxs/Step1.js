...
export default class Step1 extends PureComponent {
  ...
  render() {
    ...
    const { status, pay_limit_at, trading_count, trading_volume, order_type } = order || {};
    ...
    return (
      <div className={styles.page}>
        <Card bordered={false} className={styles.info}>
          <Meta
            title={`${trading_volume} ${currency} ${CONFIG.order_type_desc[order_type]} ${trading_count} BTC`}
            // description="中国"
          />
          <DescriptionList style={{ marginTop: 15 }} size="large" col="1">
            ...
            {order_status === 'wait_pay' && (
              <Description term="付款倒计时">
                {pay_limit_at && <CountDown target={new Date().getTime() + pay_limit_at * 1000} formatstr="mm:ss" />}
              </Description>
            )}
