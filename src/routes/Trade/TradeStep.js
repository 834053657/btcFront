...
export default class TradeStep extends PureComponent {
  ...
  orderSteps= {
    "wait_pay": {
      ...
      renderButtons: () => {
        return this.checkIsBuyer() ? (
          [
            <Button key="ok" type="primary" onClick={this.handlePay}>确认支付</Button>,
            <Button key="cancel" style={{ marginLeft: 25 }} onClick={this.handleShowCancelModal}>取消订单</Button>
          ]
        ) : null
      }
    },
    "wait_release": {
      ...
      renderButtons: () => {
        ...
        if(!this.checkIsBuyer()) {
          buttons.push(<Button key="ok" type="primary" style={{ marginRight: 25 }} onClick={this.handleRelease}>确认释放</Button>)
        }
        buttons.push(<Button key="cancel"  onClick={this.handleAppeal}>申述</Button>)
    ...
    "done": {
      ...
      renderButtons: () => {
        return <Button key="cancel" onClick={this.handleShowAppealModal} >申述</Button>;
      }
    },
    "cancel": {
      steps: [
    ...
    "appeal": {
      ...
      renderButtons: () => {
        ...
        if(this.checkIsBuyer()) {
          buttons = <Button key="cancel" onClick={this.handleShowCancelModal}>取消订单</Button>;
        }else {
          buttons = <Button key="ok" type="primary" onClick={this.handleRelease}>确认释放</Button>;
  ...
  checkIsBuyer = () => {
    ...
    return (id === trader.id && order.order_type === 1) || (id !== trader.id && order.order_type === 2)
      ...
      <PageHeaderLayout className="ant-layout-content" breadcrumbList={breadcrumbList}>
        <Card bordered={false}>
          <Steps current={current} className={styles.steps}>
            {
              map(currentObj.steps, item => <Step key={item.key} title={item.text} status={item.status} /> )
            }
          </Steps>
